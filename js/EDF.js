...
$(function(){
	...
	for(var i=max_period; i<max_lcm; i+=max_period){
		...
		for(var j=0; j<processes_Array.length; j++){
			if(i%processes_Array[j]['period'] !== 0){
				console.log(processes_Array[j]['period']+ " "+ i)
				is_lcm = 0
				break
			}
		}
	...
	for(var i=0; i<lcm; i++){
		...
		let earliest_deadline = undefined
		let earliest_deadline_index = undefined

		for(var j=0; j<processes_Array.length; j++){
			if((i/processes_Array[j]['period'] >= processes_Array[j]['times_execcuted']+processes_Array[j]['missed_deadlines'])){
				if(earliest_deadline === undefined || (processes_Array[j]['period']-i%processes_Array[j]['period'])<earliest_deadline){
					earliest_deadline_index = j
					earliest_deadline = processes_Array[j]['period']-i%processes_Array[j]['period']
				}
			}
		}

		if(earliest_deadline_index !== undefined){
			time_table[i]['running_process'] = processes_Array[earliest_deadline_index]['name']
			processes_Array[earliest_deadline_index]['in_progress_time'] += 1
			if(processes_Array[earliest_deadline_index]['in_progress_time'] == processes_Array[earliest_deadline_index]['execution_time']){
				processes_Array[earliest_deadline_index]['in_progress_time'] = 0
				processes_Array[earliest_deadline_index]['times_execcuted'] += 1
			}
		}
	...
	let HTMLTable = "<table id='time-table' class='table'><thead><tr><th class='text-center'>Time</th><th class='text-center'>Running Process</th><th class='text-center'>Deadlines</th></tr></thead><tbody>"
